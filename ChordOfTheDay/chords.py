...
import smtplib, ssl
from chord_database import subscribers
...
""" + str(chord_of_day["positions"]).strip('[]').replace("'","") + """

Its root note is """ + chord_of_day["root"] + """, which has the following major scale:
""" + str(chord_of_day["major_scale"]).strip('[]').replace("'","") + """

And the notes that compose """ + chord_of_day["name"] + """ are:
""" + str(chord_of_day["notes"]).strip('[]').replace("'","") + """
...
You can use this chart to help you find the notes: https://alvaroaguirre.com/guitar_chart

Enjoy,
Alvaro Aguirre
alvaroaguirre.com

If you want to unsubscribe from Chord of the Day, please click here: https://alvaroaguirre.com/unsubscribe
"""
...
with smtplib.SMTP_SSL(smtp_server, port, context=context) as server:
    ...
    for sub in subscribers:
        server.sendmail(sender_email, sub, message)
